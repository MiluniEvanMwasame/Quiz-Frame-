...
interface Styles {
  uContainer: React.CSSProperties;
  xmtpContainer: React.CSSProperties;
  btnXmtp: React.CSSProperties;
  HomePageWrapperStyle: React.CSSProperties;
  ButtonStyledStyle: React.CSSProperties;
}

const InboxPage: React.FC = () => {
  const [signer, setSigner] = useState<ethers.Signer | null>(null);
  const [walletConnected, setWalletConnected] = useState<boolean>(false); // Add state for wallet connection
  ...
  const getAddress = async (signer: ethers.Signer | null): Promise<string | undefined> => {
    try {
